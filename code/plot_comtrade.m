...
function plot_residual_breakdown(dataStruct, plotSpec)
    ...
    residualCount = size(plotSpec.residualMatrix, 2);
    if residualCount == 21
        groups = { ...
            1, ...
            2:4, ...
        ...
        groups = chunk_indices(residualCount, 6);
        titles = cell(1, numel(groups));
        for idx = 1:numel(groups)
            titles{idx} = sprintf('Active Residuals %d', idx);
        end
    end
...
function groups = chunk_indices(count, maxGroups)
    edges = round(linspace(0, count, min(maxGroups, count) + 1));
    groups = cell(1, numel(edges) - 1);
    for idx = 1:(numel(edges) - 1)
        startIdx = edges(idx) + 1;
        endIdx = edges(idx + 1);
        groups{idx} = startIdx:endIdx;
    end
end
